...
Botni guruhingizga qo'shish uchun pastdagi tugmani bosing va admin huquqlarini bering.
    ...
    # Add this method to your handlers.py BotHandlers class

async def debug_broadcast_command(self, message: Message):
    """Debug broadcast - send test message to all groups immediately"""
    if message.from_user.id != Config.SUPERADMIN_ID:
        await message.answer("❌ Sizda bu buyruqni ishlatish huquqi yo'q!")
        return
    ...
    if len(parts) < 2:
        await message.answer(
            "📢 **Debug Broadcast**\n\n"
            "**Foydalanish:** `/debug_broadcast Your message here`\n\n"
            "Bu buyruq xabarni darhol barcha guruhlarga yuboradi!"
        )
        return
    ...
    try:
        ...
        status_msg = await message.answer(
            f"📢 **Debug Broadcast Boshlandi**\n\n"
            f"Xabar: {broadcast_text[:100]}{'...' if len(broadcast_text) > 100 else ''}\n"
        ...
        for group in groups:
            try:
                await self.bot.send_message(group['id'], broadcast_text)
                sent_count += 1
                await asyncio.sleep(0.5)  # Rate limit protection
                
            except Exception as e:
                error_count += 1
                error_details.append(f"• {group['title']}: {str(e)[:50]}...")
                logging.warning(f"Failed to send to group {group['id']}: {e}")
        
        # Final report
        result_text = f"✅ **Debug Broadcast Yakunlandi**\n\n"
        result_text += f"📊 **Natijalar:**\n"
        result_text += f"• Muvaffaqiyatli: {sent_count}/{len(groups)}\n"
        result_text += f"• Xatoliklar: {error_count}\n\n"
        
        if error_details:
            result_text += f"❌ **Xatolik tafsilotlari:**\n"
            result_text += "\n".join(error_details[:5])  # Show first 5 errors
            if len(error_details) > 5:
                result_text += f"\n... va yana {len(error_details) - 5} ta xatolik"
        
        await status_msg.edit_text(result_text)
        
    except Exception as e:
        logging.error(f"Debug broadcast error: {e}")
        await message.answer(f"❌ Xatolik: {str(e)}")

# Also add this to the _setup_handlers method in handlers.py:
# self.router.message.register(self.debug_broadcast_command, Command("debug_broadcast"), F.chat.type == ChatType.PRIVATE)
